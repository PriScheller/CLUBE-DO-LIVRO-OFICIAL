...
// Elementos do DOM - Dados do Livro
...
// Elementos do DOM - Inputs da Ficha
...
// Elementos do DOM - Comentários e Ações
...
// 3. BUSCA DADOS DO FIRESTORE E PREENCHE A TELA
async function puxarDadosDoLivro() {
    try {
        ...
        if (docSnap.exists()) {
            ...
            // Textos Críticos da Capa e Título
            ...
            // Setar Formato de Leitura Ativo
...
// 4. CONTROLE INTERATIVO DE ESTRELAS
...
// 5. CÁLCULO DE PORCENTAGEM EM TEMPO REAL
...
// 7. MOTOR DO EDITOR DE TEXTO FORMATADO E EMOJIS
function configurarFormatadorTexto() {
    ...
    // Inserção Direta de Emojis Corrigida
...
// 9. RENDERIZAR E INTERPRETAR HTML DOS COMENTÁRIOS NO HISTÓRICO
    ...
    lista.forEach((coment) => {
        ...
        div.innerHTML = `
            <div class="meta-comentario">
                ...
                <button class="btn-deletar-comentario" data-id="${coment.id}"><i class="fa-solid fa-trash"></i></button>
            </div>
...
if (btnSalvarFichaGlobal) {
    btnSalvarFichaGlobal.addEventListener('click', async () => {
        ...
        try {
            ...
            console.error("Erro ao salvar documento global:", error);
            alert("Não foi possível salvar os dados da ficha.");
        } finally {
            btnSalvarFichaGlobal.disabled = false;
            btnSalvarFichaGlobal.textContent = "Salvar Alterações da Ficha";
        }
    });
}