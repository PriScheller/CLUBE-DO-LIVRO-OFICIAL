...
const DOM = {
    ...
    // Votação Atualizada (Título + Prazo por Data + Histórico)
...
if (DOM.btnIrSugestoes) {
    DOM.btnIrSugestoes.addEventListener('click', () => {
        if (idClubeAtual) window.location.href = `clube-sugestoes.html?id=${idClubeAtual}`;
    });
}
...
// Sistema Automático de Verificação de Prazos (Armazenamento permanente)
async function verificarEArquivarVotacaoExpirada(votacao) {
    ...
    // Configurando data de hoje truncada (apenas ano-mes-dia) para bater com o input date
    const hoje = new Date();
    hoje.setHours(0,0,0,0);
    
    // Tratando a string do prazo 'AAAA-MM-DD' recebida do input
    const [ano, mes, dia] = votacao.prazoLimite.split('-');
    const dataLimite = new Date(ano, mes - 1, dia);

    // Se o dia de hoje já passou do dia limite estabelecido, joga pro histórico de encerradas
    if (hoje > dataLimite) {
        try {
            await updateDoc(doc(doc(db, "clubes", idClubeAtual)), {
                historicoVotacoes: arrayUnion(votacao),
                votacaoAtiva: null // Limpa o espaço principal
            });
            // Recarrega os dados locais atualizados após mutação segura
...
function renderizarVotacaoAtiva(votacao) {
    ...
    // Exibe a pergunta e o prazo formatado
    ...
    DOM.containerVotacao.querySelectorAll('.btn-votar-opcao').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            const indexOpcao = parseInt(e.target.dataset.index);
            await registrarVotoNoBanco(indexOpcao, votacao);
        });
    });
}

// Criação da cédula com Título, até 3 alternativas e Prazo (Apenas Data)
if (DOM.formAdicionarOpcaoVoto) {
    DOM.formAdicionarOpcaoVoto.addEventListener('submit', async (e) => {
        ...
        const prazoDefinido = DOM.inputVotoPrazo.value; // Coleta string 'AAAA-MM-DD'
...
async function registrarVotoNoBanco(index, votacao) {
    ...
    // Garante voto único limpando o ID do usuário de todas alternativas da cédula
...
// Renderizador do Arquivo de Votações Encerradas (Armazenamento Histórico)
function renderizarHistoricoVotacoes(listaVotacoesAntigas) {
    ...
    listaVotacoesAntigas.forEach(vot => {
        ...
        bloco.style.cssText = "background: #F9F5F0; padding: 10px; border-radius: 6px; border-left: 4px solid #BF9B7A; font-size: 0.85rem;";
...
function renderizarComentarios(comentariosArray) {
    ...
    comentariosFiltrados.forEach(com => {
        ...
        itemComentario.style.cssText = "background: #FFF; padding: 6px 10px; border-radius: 6px; border: 1px solid #F0E6D8; font-size: 0.85rem;";
